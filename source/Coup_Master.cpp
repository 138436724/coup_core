...
COUP::Coup_Master::Coup_Master(uint32_t id, int num)
    : id(id), num_players(num), game_status(COUP::GAME_STATUS::JOIN), all_players({}), all_actions({}),
...
void COUP::Coup_Master::handleDOUBT(const Coup_Action *action)
{
    ...
    else if (pre_action->action == COUP::ROLE_ACTION::BLOCK)
    {
        ...
        else if (pre_pre_action->action == COUP::ROLE_ACTION::STEAL)
        {
            ...
            identity_was_doubt.push_back(COUP::ROLE_IDENTITY::AMBASSADOR);
            has_identity = dst_player->checkIdentity(COUP::ROLE_IDENTITY::CAPTAIN) ||
                           dst_player->checkIdentity(COUP::ROLE_IDENTITY::AMBASSADOR);
        }
    }
...
void COUP::Coup_Master::checkGameStatus()
{
    auto p = all_players | std::views::filter([](auto player)
                                              { return player.second->isPlayerAlive(); });
    if (std::ranges::distance(p) == 1)
    {
...
{
    if (game_status == COUP::GAME_STATUS::JOIN)
    {
...
{
    auto player = all_players.find(player_id);
    if (player != all_players.end())
    {
...
bool COUP::Coup_Master::addAction(uint32_t src_player_id, uint32_t dst_player_id, COUP::ROLE_ACTION action, bool round_end)
{
    ...
    if ((action == COUP::ROLE_ACTION::COUP && coins_num < 7) ||
        ...
        game_status == COUP::GAME_STATUS::OVER)
    {
        return false;
    }
...
COUP::GAME_STATUS COUP::Coup_Master::getGameStatus() const
{
    return game_status;
}
...
callbackFunc COUP::Coup_Master::callback = [](uint32_t, uint32_t, int, COUP::ROLE_IDENTITY *) -> uint8_t
{ return 0; };